...
- `value_wo` (String) Write-only variable value. Use instead of `value` when working with ephemeral values. Not stored in state. Requires `value_wo_version` to trigger updates.
- `value_wo_version` (Number) Version number for `value_wo`. Change this number to apply the current `value_wo` during an update.
- `var_set_id` (String) ID of the variable set this variable belongs to, in the format `varset-<RANDOM STRING>`. Cannot be used together with `workspace_id` or `environment_id`
