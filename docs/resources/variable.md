...
- `value_wo` (String) Write-only variable value. Use instead of `value` when working with ephemeral values (Terraform 1.11+). Not stored in state. Requires `value_wo_version` to trigger updates.
